...
PS_OUT PS_Main(VS_OUT input) 
{
    ...
    float4 baseColor = _BaseMap.Sample(sampler_lerp, input.uv) * color;
    float4 emission = _EmissionMap.Sample(sampler_lerp, input.uv);

    output.normal = float4(N, 1.0f);
    output.color = baseColor + emission*1.5f;
